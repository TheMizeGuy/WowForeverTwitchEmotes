...
-- WoW Forever (game type "camelot", interface 16001) runs on the retail 12.x engine. It has the
-- Midnight chat messaging restrictions and the ChatFrame.OnEditBoxPreSendText hook even though
-- its build number (1.60.x) is far below 120000, so detect the feature instead of the version.
-- Classic-engine clients have neither, and keep the old C_ChatInfo.SendChatMessage wrapper.
local hasChatRestrictions = (C_ChatInfo.InChatMessagingLockdown ~= nil)
local isMidnight = hasChatRestrictions or (select(4, GetBuildInfo()) >= 120000)
local InChatMessagingLockdown = (C_ChatInfo and C_ChatInfo.InChatMessagingLockdown) or function()
	return false
end
-- 12.x moved the chat filter API into ChatFrameUtil. The old ChatFrame_* globals only exist while
-- the loadDeprecationFallbacks CVar is on, so prefer the namespaced functions when present.
...
-- Both frames exist at load on Forever (Blizzard_UIPanels_Game and Blizzard_MailFrame are not
-- load-on-demand there), so these guards only keep a client that lacks one from aborting the
-- rest of this file. They do not install the hook later if the frame appears after load.
...
-- Blizzard's own MailFrame still calls the SendMail global on Forever, so this guard is only for
-- a client that has moved mail sending into a namespace.
if SendMail then
	local sm = SendMail;
...
if isMidnight then
	EventRegistry:RegisterCallback( --? Reaffirm we are not sending emotes during combat, as that can cause issues with the Midnight messaging restrictions.
		"ChatFrame.OnEditBoxPreSendText",
		function(self, editBox)
			if not Emoticons_Settings["ENABLE_CLICKABLEEMOTES"] then return end

			local msg = editBox:GetText()
			if not msg or issecretvalue(msg) or msg:match("^%s*$") then return end

			local newMsg = TwitchEmotes_Message_StripEscapes(msg)
			if newMsg ~= msg then
...
-- On the 12.x engine the pre-send hook above already strips the edit box before Blizzard calls
-- C_BattleNet.SendWhisper, and the BNSendWhisper global is only a CVar-gated deprecated alias.
if BNSendWhisper and not isMidnight then
...
function Emoticons_UpdateChatFilters()
	for k, v in pairs(Emoticons_Settings) do
		if type(k) == "string" and k:match("^CHAT_MSG_") then
			if (v) then
				AddMessageEventFilter(k, Emoticons_MessageFilter);
			else
				RemoveMessageEventFilter(k, Emoticons_MessageFilter);
			end
		end
	end
end
...
function Emoticons_OnEvent(self, event, ...)
	if (event == "ADDON_LOADED" and select(1, ...) == "TwitchEmotes") then
		...
		TwitchEmotesUpdateFrame = CreateFrame("Frame", "TwitchEmotes_EventFrame", UIParent)
		TwitchEmotesUpdateFrame:SetScript('OnUpdate', TwitchEmotes_Update)
...
function Emoticons_OptionsWindow_OnShow(self)
	...
	if Emoticons_Settings["MINIMAPBUTTON"] then
		_G["$showMinimapButtonButton"]:SetChecked(true);
	end

	if Emoticons_Settings["LARGEEMOTES"] then
		_G["$showLargeEmotesButton"]:SetChecked(true);
	end

	if Emoticons_Settings["ENABLE_CLICKABLEEMOTES"] then
		_G["$showClickableEmotesButton"]:SetChecked(true);
	end

	if Emoticons_Settings["ENABLE_AUTOCOMPLETE"] then
		_G["$autocompleteCheckButton"]:SetChecked(true);
	end

	if Emoticons_Settings["AUTOCOMPLETE_CONFIRM_WITH_TAB"] then
		_G["$autocompleteUseTabToComplete"]:SetChecked(true);
	end
	

	-- _G["$autocompleteCheckButton"].tooltipText = "Start with a ':' to show a list of emotes.";

	_G["$autocompleteUseTabToComplete"].tooltipText = "This will disable cycling the selected suggestion with tab, the arrow keys will still work.";
...
function TwitchEmotes_Update(self, elapsed)
	...
	if nChatFrames ~= TwitchEmotes_N_CHATFRAMES then
		...
		TwitchEmotes_SetupChatFrames();
	end
	
	if Emoticons_Settings["ENABLE_ANIMATEDEMOTES"] then
		TwitchEmotesAnimator_OnUpdate(self, elapsed);
	end
end

function TwitchEmotes_SetupChatFrames() 
	for _, frameName in pairs(CHAT_FRAMES) do
		local frame = _G[frameName]
