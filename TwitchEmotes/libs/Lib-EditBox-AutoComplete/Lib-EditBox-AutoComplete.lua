...
-- The retail 12.x engine (which WoW Forever shares) cycles whisper targets from a secure
-- OnTabPressed handler that never consults the ChatEdit_GetNextTellTarget swap used below.
-- Blizzard keeps ChatEdit_CustomTabPressed global as the sanctioned addon hook: returning true
-- from it skips the whisper-target cycling while the emote suggestion box owns the Tab key.
local previousCustomTabPressed = ChatEdit_CustomTabPressed
function ChatEdit_CustomTabPressed(editBox)
	if EditBoxAutoCompleteBox and EditBoxAutoCompleteBox:IsShown() and EditBoxAutoCompleteBox.parent == editBox then
		return true
	end
	if previousCustomTabPressed then
		return previousCustomTabPressed(editBox)
	end
end
	...
	if (settings.useArrowButtons) then
		editbox:SetScript("OnKeyDown", function(editbox, key)
			...
			if (EditBoxAutoCompleteBox:IsShown() and (EditBoxAutoCompleteBox.parent == editbox)) then
				-- Kept for Classic-engine clients, where this global still drives Tab. The 12.x
				-- engine ignores it and is handled by ChatEdit_CustomTabPressed above.
				if key == "TAB" then
